...
static int create_recorder(struct buffer_instance *instance,
			   int cpu, enum trace_type type, int *brass);
...
static void flush_threads(void)
{
	...
	for_all_instances(instance) {
		for (i = 0; i < instance->cpu_count; i++) {
			...
			ret = create_recorder(instance, i,
...
void start_threads(enum trace_type type, int global)
{
	...
	for_all_instances(instance) {
		...
		for (x = 0; x < instance->cpu_count; x++) {
			...
			fflush(stdout);
			pid = pids[i++].pid = create_recorder(instance,
							      x, type, brass);
			if (brass)
				close(brass[1]);
			if (pid > 0)
...
void trace_record (int argc, char **argv, struct tracecmd_msg_handle *msg_handle)
{
	...
	for (;;) {
		...
		case 'B':
			...
			add_instance(instance, local_cpu_count);
			if (profile)
				instance->flags |= BUFFER_FL_PROFILE;
			break;
		case 'k':
			keep = 1;
			break;
	...
	for_all_instances(instance) {
		...
		if (!manual && instance->flags & BUFFER_FL_PROFILE)
